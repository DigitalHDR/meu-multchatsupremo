...
function fillMessageElement(messageEl, data) {
  messageEl.className = 'chat-message platform-' + data.platform;
  if (data.pinned) messageEl.classList.add('is-pinned');

  const badge = messageEl.querySelector('.platform-badge');
  ...
  renderMessageContent(messageEl.querySelector('.message-text'), data);
}

function setPinnedMessage(data) {
  if (!pinnedSlot || !template) return;

  if (!data) {
    pinnedSlot.innerHTML = '';
    pinnedSlot.classList.add('hidden');
  ...
  if (!messageEl) return;
  fillMessageElement(messageEl, { ...data, pinned: true });
  pinnedSlot.innerHTML = '';
  pinnedSlot.appendChild(messageEl);
  pinnedSlot.classList.remove('hidden');
...
function addMessage(data, fromHistory) {
  ...
  fillMessageElement(messageEl, data);
...
function sendStreamerPin(text) {
  const message = String(text || '').trim();
  if (!message) return false;

  if (activeWs && activeWs.readyState === WebSocket.OPEN) {
    activeWs.send(JSON.stringify({ type: 'streamer-pin', message }));
    return true;
  }

  fetch('/api/streamer-pin', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ message }),
  }).catch(() => {});
...
function sendStreamerUnpin() {
  if (activeWs && activeWs.readyState === WebSocket.OPEN) {
    activeWs.send(JSON.stringify({ type: 'streamer-unpin' }));
    return;
  }

  fetch('/api/streamer-pin', { method: 'DELETE' }).catch(() => {});
}
...
function setupStreamerComposer() {
  ...
  const pinBtn = document.getElementById('streamer-pin');
  unpinBtn = document.getElementById('streamer-unpin');
  if (!composer || !input || !sendBtn || !pinBtn || !unpinBtn) return;

  composer.classList.remove('hidden');
  composer.setAttribute('aria-hidden', 'false');
  pinBtn.classList.add('secondary');
  unpinBtn.classList.add('danger');
  ...
  const submit = () => {
    if (sendStreamerMessage(input.value)) clearAndFocus();
  };

  const pin = () => {
    if (sendStreamerPin(input.value)) clearAndFocus();
  };
